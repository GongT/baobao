...
const defaultImport = /^import (?<def>[a-z0-9_$]+) from '(?<fro>[^.].+)';$/img;
const namedImport = /^import { (?<nam>[a-z0-9_$ ,]+) } from '(?<fro>[^.].+)';$/img;
const namespaceImport = /^import \* as (?<ns>[a-z0-9_$]+) from '(?<fro>[^.].+)';$/img;
const defaultWithNamespaceImport = /^import (?<def>[a-z0-9_$]+), \* as (?<ns>[a-z0-9_$]+) from '(?<fro>[^.].+)';$/img;
const defaultWithNamedImport = /^import (?<def>[a-z0-9_$]+), { (?<nam>[a-z0-9_$ ,]+) } from '(?<fro>[^.].+)';$/img;
const regTypes = [
	defaultImport,
	namedImport,
	namespaceImport,
	defaultWithNamespaceImport,
	defaultWithNamedImport,
];
const jsExt = /\.js$/;
const dewExt = '.dew.js';
