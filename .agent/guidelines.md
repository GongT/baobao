...
- Find out which package manager is used before installing. never execute different package manager in the same project. for example, if you find `pnpm-lock.yaml`, then you should use `pnpm` or `pnpx`, run `npm` or `yarn` may damage the project. Fail if you finally not able to find any package manager, never assume "not set is using the default one".
...
# Language version:
- You must use the latest edge version of any language.
- Never write any code or notification for backward compatibility.
- When editing code, if you found the code is using old grammar or API, you should update them to the latest version, and notify user about it.

Examples:
- `const fs = require('fs')` should be updated to `import fs from 'node:fs/promises'`.
- `readFile(f, callback)` should be updated to `await readFile(f)`.
- `Promise.resolve().then(() => ...)` can replaced with `Promise.try(() => ...)`.
- `"hello {}".format(val)` should be updated to `f"hello {val}"`.

# When programming language of user question is unclear:

- Inspect current workspace, assume the question is about that.
- If still unclear, ask user to specify the programming language, and recommend to write a AGENTS.md for clarification.
