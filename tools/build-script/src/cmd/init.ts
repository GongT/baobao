...
async function modifyPackageJson(ctx: BuildContext) {
	...
	addOrFail(packageJson.scripts, 'cleanup', 'build-script cleanup');
	addOrFail(packageJson.scripts, 'prepublish', 'build-script prepublish');
...
async function createBuildJson(ctx: BuildContext) {
	...
	ctx.addAction('cleanup', ['cleanup-lib']).title = 'Cleanup build result';
	ctx.addAction('prepublish', ['@build'], ['cleanup']).title = 'Prepare for publish package';
	ctx.addAction('publish', ['yarn-publish'], ['prepublish']).title = 'Publish package';
